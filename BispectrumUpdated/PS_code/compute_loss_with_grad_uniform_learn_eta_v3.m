function [loss, grad] = compute_loss_with_grad_uniform_learn_eta_v3(w,sqrt_g,d,RandomDilationOpts)
...
loss = sum((g - dilated_g - h).^2)/eta^2; 
...
grad_sqrt_g = 4*sqrt_g(1:end-1).*( f - shrink_f)./eta^2;

%if strcmp(RandomDilationOpts.Normalization,'Linf')
%    shrink_g = ((C_0)^2)*interp1( w.*C_0, g, w,RandomDilationOpts.InterpolationMethod);
%elseif strcmp(RandomDilationOpts.Normalization,'L1')
%    shrink_g = interp1( w.*C_0, g, w,RandomDilationOpts.InterpolationMethod);
%end
%no_interp_idx = find((w<C_0*min(w)) | (w>C_0*max(w)));
%shrink_g(no_interp_idx)=0;
%grad_sqrt_g = 4*shrink_g.*sqrt_g(1:end-1).*f/eta^2;
...
grad_eta = (sum(2*f.*partial_f_eta)/eta^2 - 2*loss/eta);
