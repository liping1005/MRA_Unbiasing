...
if(strcmp(RandomDilationOpts.SynthesisDomain, 'Space'))
    if norm(UndilatedPowerSpectrum) > 0
        BSerror_NoDilUnbias_rel = norm(UndilatedBispectrum - unbiased_BS .*2^(3*l))/norm(UndilatedBispectrum);
        if strcmp(RandomDilationOpts.UnbiasingMethod, 'Uniform') == 1
...
elseif(strcmp(RandomDilationOpts.SynthesisDomain, 'Frequency'))
    if norm(UndilatedPowerSpectrum) > 0
        BSerror_NoDilUnbias_rel = norm(UndilatedBispectrum - unbiased_BS)/norm(UndilatedBispectrum);
        if strcmp(RandomDilationOpts.UnbiasingMethod, 'Uniform') == 1
            BSerror_rel = norm(UndilatedBispectrum - complex(est_bispectrum_real, est_bispectrum_im))/norm(UndilatedBispectrum);
        elseif strcmp(RandomDilationOpts.UnbiasingMethod, 'GeneralDerivative') == 1
            BSerror_rel = norm(UndilatedBispectrum - complex(est_bispectrum_real, est_bispectrum_im))/norm(UndilatedBispectrum);
        end 
    elseif norm(UndilatedPowerSpectrum) == 0 %for the zero signal, just compute absolute error
        BSerror_NoDilUnbias_rel = norm(UndilatedBispectrum - unbiased_BS .*2^(3*l));
        if strcmp(RandomDilationOpts.UnbiasingMethod, 'Uniform') == 1
            BSerror_rel = norm(UndilatedBispectrum - recovered_bs);
        elseif strcmp(RandomDilationOpts.UnbiasingMethod, 'GeneralDerivative') == 1
            BSerror_rel = norm(UndilatedBispectrum - recovered_bs);
...
[f_est_aligned_no_ub_aps, rel_error_inv_no_ub_aps] = invert_bs(PlotFourierTransformAvg, 2^(2*l) .* TargetPowerSpectrum,unbiased_BS.*(2^(3*l)), f, l, 'APS');
[f_est_aligned_aps, rel_error_inv_aps] = invert_bs(PlotFourierTransformAvg, UnbiasedPS, recovered_bs, f, l,'APS');

[f_est_aligned_no_ub_fm, rel_error_inv_no_ub_fm] = invert_bs(PlotFourierTransformAvg, 2^(2*l) .* TargetPowerSpectrum,unbiased_BS.*(2^(3*l)), f, l, 'FM');
[f_est_aligned_fm, rel_error_inv_fm] = invert_bs(PlotFourierTransformAvg, UnbiasedPS, recovered_bs, f, l, 'FM');