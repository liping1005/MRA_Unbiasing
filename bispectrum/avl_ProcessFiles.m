...
Y_w= repmat(flip(w)',1,length(w));
data = 4 .* BispectrumAvg +  CalculateDerivatives(BispectrumAvg, X_w, Y_w,  w(2)-w(1));
%data = 4 .* BispectrumAvg +  CalculateDerivatives(BispectrumAvg, X_w, Y_w,  1);
figure
imagesc('XData',w,'YData',w,'CData',real(BispectrumAvg))
colorbar
title('Bisp Avg')
figure
imagesc('XData',w,'YData',w,'CData',real(CalculateDerivatives(BispectrumAvg, X_w, Y_w,  w(2)-w(1))))
colorbar
title('Deriv term')
figure
